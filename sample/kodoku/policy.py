...
class PolicyMappingManager(metaclass=ABCMeta):
    ...
    def get_policy_mapping(
        ...
        Args:
            ...
            episode (EpisodeV2): Episode instance
...
class FictitiousSelfPlayManager(PolicyMappingManager):
    ...
    def get_policy_mapping(
        ...
        assert force == "blufor" or force == "redfor", (
            'agent_force_mapping_fn must return "blufor" or "redfor".'
        )
    ...
    def update_policy_configuration(
        ...
        if self.subpolicy_priority_fn is not None and match_matrix is not None:
            ...
            self.p /= np.sum(self.p)
            assert isinstance(self.p, np.ndarray) and self.p.shape == (
                self.num_subpolicies,
                self.num_subpolicies,
            ), "subpolicy_priority_fn must return (%d, %d) matrix." % (
                self.num_subpolicies,
                self.num_subpolicies,
            )
